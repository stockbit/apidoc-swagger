...
function createOutputFile(api){
    ...
    var outfile = app.options.dest + './swagger.json';

    //Check if output file exists
    var out = app.options.dest;
    var res = out.split("/");
    var file = res[res.length-2];
    var fileArr = file.split(".");
    var ext = fileArr[fileArr.length-1];
    // Alowed output file extension
    if(ext == 'js' || ext == 'json'){
        customOutput = true;
        var dest = out.replace(file+'/','');
        outfile = dest+file;
    }

    if ( ! app.options.simulate && ! customOutput)
        fs.mkdirsSync(app.options.dest);

    //Write swagger
    app.log.verbose('write swagger json file: ' + app.options.dest + 'swagger.json');
    if( ! app.options.simulate)
        fs.writeFileSync(outfile, api.swaggerData); 
}
